...
router.get("/users", async (req, res) => {
    ...
    try {
        let query = `SELECT id, username, email, phone_number, profile_picture, bio FROM users WHERE username ILIKE $1 OR email ILIKE $1 ORDER BY id LIMIT $2 OFFSET $3`;
...
router.put("/edit/:id", async (req, res) => {
    const { id } = req.params;
    const { username, email, phoneNumber, profilePicture, bio } = req.body;

    if (!username && !email && !phoneNumber && !profilePicture && !bio) {
